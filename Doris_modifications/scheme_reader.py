"""
To prevent merging errors on github, this is the file that I write my code in.
"""
...
def scheme_read(src):
    ...
    if val == "nil":
        ...
        #return quote special form if current token is single quote
...
def read_tail(src):
    ...
    """
    # for an dotted (ill-formed) list
    #assuming ( is popped by scheme_read
    try:
        if src.current() is None:
            raise SyntaxError("unexpected end of file")
        if src.current() == ")": #last value in Buffer
            src.pop()
            return nil
        "*** YOUR CODE HERE ***"
        if src.current()== ".":
            src.pop()
            val=scheme_read(src)#val stores the list without the last element
            if src.current()!=")":
